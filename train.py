...
def train(
    ...
    for idx, batch in enumerate(dataloader):
        ...
        if accelerator.sync_gradients:
            accumulated_metrics = accelerator.reduce(
                accumulated_metrics, reduction="mean"
            )

            if accelerator.is_main_process:
                accumulated_metrics = {
                    k: v.detach().item() / config.gradient_accumulation_steps
                    if isinstance(v, torch.Tensor)
                    else v / config.gradient_accumulation_steps
                ...
                progress.update(
                    n=num_samples, **{k: v for k, v in accumulated_metrics.items()}
                )

                # log summary
                if accumulated_metrics["loss"] < min_loss:
                    accelerator.trackers[0].tracker.summary["min_loss"] = (
                        accumulated_metrics["loss"]
                    )
                if accumulated_metrics["accuracy"] > max_accuracy:
                    accelerator.trackers[0].tracker.summary["max_acc"] = (
                        accumulated_metrics["accuracy"]
                    )
...
def main_train(config: TrainConfig):
    ...
    config.save_path += (
        f"{run_name}_{datetime.now().isoformat(sep='_', timespec='seconds')}"
    )
    ...
    if config.debug:
        ...
        with accelerator.main_process_first():
            ...
            dataset = load_from_disk(config.data_config.data_path).select(
                range(num_samples)
            )
    ...
    optim_groups = configure_optimizers(
        accelerator.unwrap_model(model), config, accelerator.is_main_process
    )
    ...
    if config.optimizer == "AdamW":
        ...
        optimizer = torch.optim.SGD(params=optim_groups, lr=config.lr, momentum=0.9)
    else:
