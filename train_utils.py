...
def configure_optimizers(model, train_config: TrainConfig, is_main: bool):
    ...
    hc_params_static = ["static_beta", "static_alpha"]
    hc_params_dynamic = [
        "dynamic_alpha_fn",
        "dynamic_alpha_scale",
        "dynamic_beta_fn",
    ...
    lime_params_no_decay = (
        [] if train_config.lime_config.wd_on_static_weights else ["static_weights"]
    )
    lime_params_decay = (
        ["static_weights", "dynamic_proj"]
        if train_config.lime_config.wd_on_static_weights
        else ["dynamic_proj"]
    )
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            if (
                pn.endswith("bias")
                or pn in hc_params_static
                or pn in lime_params_no_decay
            ):
                no_decay.add(fpn)
            elif (
                pn.endswith("weight")
                and isinstance(m, whitelist_weight_modules)
                or pn in lime_params_decay
            ):
    ...
    optim_groups = [
        {
            "params": [
                ...
                if pn in param_dict and pn not in large_lr_router_params
            ],
        ...
        },
        {
            "params": [
                param_dict[pn]
                for pn in sorted(list(no_decay))
                if pn in param_dict and pn in large_lr_router_params
            ],
            "weight_decay": 0.0,
