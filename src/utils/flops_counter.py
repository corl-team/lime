...
def output_shape_flops_counter(
    inputs: List[Any], outputs: List[Any], multiplicator: int = 1
) -> Number:
    """
    Count flops for operations maybe including broadcasting.
    """
    outputs_shapes = [
        get_shape(v) for v in outputs
    ]  # calculating outputs shape to take in mind broadcasting ops
    num_elements = prod(outputs_shapes[0])
    return num_elements * multiplicator


def input_shape_flops_counter(
    inputs: List[Any], outputs: List[Any], multiplicator: int = 1
) -> Number:
    """
...
def get_model_flops(model: PreTrainedModel, input_shape: Tuple = (1, 2048)) -> int:
    ...
    input_ids = torch.randint(
        low=0, high=model.config.vocab_size, size=input_shape, device=model.device
    )
