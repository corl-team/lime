...
def tokenize_and_save_dataset(
    ...
    dataset = load_dataset(
        dataset_name, subset, split=split, cache_dir=cache_dir, num_proc=num_proc
    ).select_columns(text_field_name)
    ...
    dataset = dataset.map(
        lambda x: tokenizer(
            x[text_field_name], truncation=False, padding=False, verbose=False
        ),
        batched=True,
        num_proc=num_proc,
        remove_columns=text_field_name,
    ...
    def batched_split(batch):
        ...
        for sentence in batch["input_ids"]:
            all_input_ids += (
                [tokenizer.bos_token_id] + sentence + [tokenizer.eos_token_id]
            )
        ...
        input_ids = [
            all_input_ids[i : i + seq_len] for i in range(0, total_length, seq_len)
        ]
        labels = [
            all_input_ids[i + 1 : i + seq_len + 1]
    ...
    dataset.save_to_disk(
        f"{cache_dir}/tokenized/{dataset_name.split('/')[-1]}__{subset}/len{seq_len}_{num_tokens}tokens"
    )
